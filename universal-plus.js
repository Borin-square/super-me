(() => {
  ...
  async function analyze(payload){if(!payload.text&&!payload.imageDataUrl)return;openSheet(`<div class="loading"><span class="spinner"></span><b>Sto interpretando…</b></div>`);try{const r=await fetch('/api/parse',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)});const d=await r.json();if(!r.ok)throw new Error(d.error||'AI non disponibile');if(d.kind==='sport'&&!d.kcalExplicit)d.kcal=estimateSportKcal(d.type,d.minutes,d.intensity||'media');confirmData(d);}catch(e){alert(e.message);home();}}
  function fields(d){
