...
const entrySchema={type:'object',additionalProperties:false,required:['kind','confidence','needsReview','note','meal','title','author','kcal','p','c','f','sodiumMg','sodiumExplicit','type','minutes','distance','elevation','intensity','sys','dia','pulse','context','weight','smokingAction','coffeeCount'],properties:{
 kind:{type:'string',enum:['food','sport','pressure','weight','meditation','book','smoking','coffee']},confidence:{type:'number'},needsReview:{type:'boolean'},note:{type:'string'},meal:{type:'string',enum:['Colazione','Pranzo','Cena','Snack','']},title:{type:'string'},author:{type:'string'},kcal:{type:'number'},p:{type:'number'},c:{type:'number'},f:{type:'number'},sodiumMg:{type:'number'},sodiumExplicit:{type:'boolean'},type:{type:'string',enum:['Bici','Corsa','Calisthenics','Camminata','Calcio','']},minutes:{type:'number'},distance:{type:'number'},elevation:{type:'number'},intensity:{type:'string',enum:['bassa','media','alta','']},sys:{type:'number'},dia:{type:'number'},pulse:{type:'number'},context:{type:'string',enum:['riposo','mattina','sera','post-caffè','post-sport','altro','']},weight:{type:'number'},smokingAction:{type:'string',enum:['smoked','quit_start','']},coffeeCount:{type:'number'}
}};
...
function sanitize(e){
 const out={...e,confidence:Math.max(0,Math.min(1,+e.confidence||0)),needsReview:!!e.needsReview,note:String(e.note||'')};
 if(e.kind==='pressure'){if(!(e.sys>=70&&e.sys<=260&&e.dia>=40&&e.dia<=180)){out.needsReview=true;out.note=out.note||'Valori pressione mancanti o poco plausibili.';}}
 if(e.kind==='weight'){if(!(e.weight>=30&&e.weight<=300)){out.needsReview=true;out.note=out.note||'Peso mancante o poco leggibile.';}}
 if(e.kind==='meditation'&&!(e.minutes>0&&e.minutes<=300)){out.needsReview=true;out.note=out.note||'Durata meditazione mancante.';}
 if(e.kind==='sport'&&(!e.type||!(e.minutes>0))){out.needsReview=true;out.note=out.note||'Attività o durata mancanti.';}
