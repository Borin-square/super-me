...
const schema = {
  ...
  required: [
    "kind","meal","title","author","kcal","kcalExplicit","p","c","f","estimated",
    "type","minutes","distance","elevation","intensity","sys","dia","pulse","context",
    "weight","smokingAction"
  ],
  properties: {
...
function validate(parsed) {
  ...
  if (parsed.kind === "food") {
    ...
    return {
      ...
      p: Math.round(parsed.p * 10) / 10,
      c: Math.round(parsed.c * 10) / 10,
      f: Math.round(parsed.f * 10) / 10,
      estimated: true
    };
  }
  ...
  if (parsed.kind === "sport") {
    ...
    return {
      ...base,
      type: parsed.type,
      minutes: Math.round(parsed.minutes),
      distance: parsed.distance > 0 ? parsed.distance : null,
      elevation: parsed.elevation > 0 ? Math.round(parsed.elevation) : null,
      intensity: parsed.intensity || "media",
      kcal: parsed.kcal > 0 ? Math.round(parsed.kcal) : 0,
      kcalExplicit: parsed.kcalExplicit === true
    };
  }
  ...
  if (parsed.kind === "pressure") {
    ...
    return {
      ...base,
      sys: Math.round(parsed.sys),
      dia: Math.round(parsed.dia),
      pulse: parsed.pulse > 0 ? Math.round(parsed.pulse) : null,
      context: parsed.context || "riposo"
    };
  }
...
Input utente:\n${text || "Analizza la foto e identifica quale dato personale l'utente sta registrando."}`;
    ...
    const response = await fetch("https://api.openai.com/v1/responses", {
      ...
      body: JSON.stringify({
        ...
        text: {
          format: {
            type: "json_schema",
            name: "super_me_universal_entry",
            description: "Dato universale strutturato per Super Me.",
            strict: true,
            schema
          }
        },
    ...
    try {
      return json(res, 200, validate(JSON.parse(raw)));
    } catch (error) {
