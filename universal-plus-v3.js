(() => {
  ...
  function sportKcal(type,minutes,intensity){if(!MET[type]||!(+minutes>0))return null;const met=MET[type]?.[intensity]||MET[type].media;return Math.max(1,Math.round(met*currentWeight()*(+minutes||0)/60))}
  ...
  function entryFields(e,i){
    ...
    else if(e.kind==='sport')body=`<div class="field"><label>ATTIVITÀ</label><input data-f="type" data-i="${i}" value="${esc(e.type||'')}"></div><div class="grid2"><div class="field"><label>MINUTI · OPZIONALE</label><input data-f="minutes" data-i="${i}" type="number" min="0" value="${e.minutes||''}"></div><div class="field"><label>DISTANZA KM</label><input data-f="distance" data-i="${i}" type="number" step=".01" min="0" value="${e.distance||''}"></div></div><div class="field"><label>DETTAGLIO</label><input data-f="title" data-i="${i}" value="${esc(e.title||'')}"></div><div class="field"><label>INTENSITÀ</label><select data-f="intensity" data-i="${i}"><option value="bassa" ${e.intensity==='bassa'?'selected':''}>Bassa</option><option value="media" ${(!e.intensity||e.intensity==='media')?'selected':''}>Media</option><option value="alta" ${e.intensity==='alta'?'selected':''}>Alta</option></select></div>${e.minutes>0&&MET[e.type]?'<div class="notice">Le calorie verranno stimate da durata, intensità e peso.</div>':'<div class="notice">Puoi salvare anche senza durata. Se non abbiamo dati sufficienti, le calorie restano vuote.</div>'}`;
  ...
  function commit(entries){
    ...
    for(let i=0;i<entries.length;i++){
      ...
      if(e.kind==='pressure'){const sys=+val(i,'sys'),dia=+val(i,'dia');if(sys<70||dia<40)return alert('Controlla i valori della pressione.');data.pressure||=[];data.pressure.push({id:uid(),sys,dia,pulse:+val(i,'pulse')||null,context:e.context||'riposo',createdAt:now})}
        ...
        const type=String(val(i,'type')).trim(),minutes=Math.max(0,+val(i,'minutes')||0),distance=Math.max(0,num(val(i,'distance'))),title=String(val(i,'title')).trim(),intensity=val(i,'intensity')||'media';
        if(!type)return alert('Controlla il nome dell’attività.');
        const kcal=sportKcal(type,minutes,intensity);
        data.sport||=[];data.sport.push({id:uid(),type,minutes:Math.round(minutes),distance:distance||null,elevation:e.elevation>0?e.elevation:null,intensity,title:title||null,kcal:kcal||0,kcalEstimated:!!kcal,weightUsed:kcal?currentWeight():null,createdAt:now})
      }
